...
$invRoleLower = strtolower(trim($invUserRole));
$invCompanyId = (int)($_SESSION['user']['company_id'] ?? 0);
$showTravelSessionsCta = in_array($invRoleLower, ['manager', 'admin', 'system_admin'], true)
    && ($invRoleLower === 'system_admin' || \App\Models\CompanyModule::isEnabled($invCompanyId, 'inventory_travel_sessions'));
?>
...
<div class="mb-5 bg-white rounded-xl border border-gray-200 shadow-sm p-4 sm:p-5">
    ...
    <?php if ($showTravelSessionsCta): ?>
    <div class="mb-5 rounded-xl border border-emerald-200 bg-gradient-to-r from-emerald-50 to-white p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 shadow-sm">
        <div class="min-w-0">
            <p class="font-semibold text-emerald-900 text-base">Travel inventory session</p>
            <p class="text-sm text-emerald-800/90 mt-1">Save a stock snapshot before you leave and when you come back—then compare what changed. Start or continue from the Travel sessions page.</p>
        </div>
        <a href="<?= htmlspecialchars(BASE_URL_PATH, ENT_QUOTES, 'UTF-8') ?>/dashboard/inventory/travel-sessions" class="shrink-0 inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-lg bg-emerald-600 text-white text-sm font-semibold shadow hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2">
            <i class="fas fa-route" aria-hidden="true"></i>
            Open travel sessions
        </a>
    </div>
    <?php endif; ?>
